...
const state = {
  ...
  listings: [],     // { listing, check, live } for everything that parsed
  loading: true,
  search: "",
  tld: null,
  sort: "price-desc",
...
async function featureListing(domain) {
  ...
  askDialog(
    ...
    `<p>Rank is sats zapped inside a rolling week. This pays
     ...
     <input type="text" id="zap-amount" inputmode="numeric" value="${entry.sats ? entry.sats + 1 : 1000}" autocomplete="off">
...
async function signProof() {
  ...
  try {
    ...
    const signed = await session.signer.signEvent(
      proofEvent({ domain: state.draft.domain, pubkey: session.pubkey, iat }),
    );
...
$("#search").addEventListener("input", (e) => {
  ...
  searchTimer = setTimeout(() => { state.search = e.target.value.trim().toLowerCase(); render(); }, 160);
});
