...
export function askDialog(title, bodyHtml, { confirmLabel = "Continue", onConfirm } = {}) {
...
export const closeDialog = () => $("#key-dialog")?.close();
export const session = { signer: null, pubkey: null };
...
export async function openConnect() {
  ...
  askDialog("Connect a key", `<div class="key-choice">
       <button class="btn btn-accent" type="button" id="use-ext" ${extension ? "" : "disabled"}>
         Browser extension
         <span>${extension ? "Alby, nos2x or similar. This page never sees your key." : "No NIP-07 extension detected in this browser."}</span>
       </button>
  ...
  $("#use-stored")?.addEventListener("click", () => {
    closeDialog();
    askDialog("Unlock", `<p>Your key is encrypted in this browser. The passphrase never leaves this page.</p>
       <input type="password" id="pass" placeholder="Passphrase" autocomplete="current-password">
       <p class="hint" id="pass-hint"></p>`, {
      confirmLabel: "Unlock",
      onConfirm: async () => {
        try {
          const signer = localSigner(await loadKey($("#pass").value));
          closeDialog();
          await adopt(signer);
        } catch (err) {
...
function createKey() {
  ...
  askDialog("Your new key", `<div class="warn">
    ...
    onConfirm: async () => {
      ...
      try {
        await storeKey(secret, $("#pass").value);
        closeDialog();
        await adopt(signer);
