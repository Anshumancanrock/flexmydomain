...
const fail = (el, message) => { el.className = "hint err"; el.textContent = message; };
...
async function createInvite() {
  ...
  if (d.side === "buyer") {
    ...
    catch (err) { fail(hint, err.message); return; }
...
function reviewSettlement(view, utxo) {
  ...
  catch (err) { out.innerHTML = row("bad", esc(err.message)); return; }
   ...
   terms themselves. */
{
  ...
  if (domain) {
    ...
    if (amount && /^\d+$/.test(amount)) $("#e-amount").value = amount;
    const seller = q.get("seller");
    if (seller) $("#e-counterparty").value = seller;
    $("#e-side").value = "buyer";
    const hint = $("#terms-hint");
    hint.className = "hint";
